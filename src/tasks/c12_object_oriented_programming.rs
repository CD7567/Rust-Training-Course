...
impl Elephant {
    fn new(name: &str, x: f64, y: f64) -> Self {
        Elephant { name: name.to_string(), x, y }
    }
}
...
pub fn demonstrate_zoo() {
    println!("=== Zoo Demonstration ===");

    let mut zoo = Zoo::new();

    zoo.add_animal(Box::new(Lion::new("Simba", 10.0, 5.0)));
    zoo.add_animal(Box::new(BritishPigeon::new("Tony", 2.0, 8.0)));
    zoo.add_animal(Box::new(Elephant::new("Dumbo", 15.0, 3.0)));
    ...
    println!("\nInitial positions:");
    for (name, pos) in zoo.positions() {
        println!("  {}: ({}, {})", name, pos.0, pos.1);
    }

    zoo.move_all(5.0, -2.0);

    println!("\nPositions after moving (5.0, -2.0):");
    for (name, pos) in zoo.positions() {
        println!("  {}: ({}, {})", name, pos.0, pos.1);
    }
}
